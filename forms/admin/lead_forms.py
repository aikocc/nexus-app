...
class LeadForm(FlaskForm):
    ...
    phone = IntegerField('Phone', validators=[DataRequired(), Length(max=20)])
    email = StringField('Email', validators=[DataRequired(), Email()])
    address = StringField('Address', validators=[DataRequired(), Length(max=200)])
    suburb = StringField('Suburb', validators=[DataRequired(), Length(max=100)])
    postcode = IntegerField('Postcode', validators=[DataRequired(), Length(max=4)])

    # Vehicle Details
    rego = StringField('rego', validators=[DataRequired(), Length(max=10)])
    rego_state = SelectField('rego state', choices=[('NSW', 'NSW'), ('VIC', 'VIC'), ('QLD', 'QLD'), ('WA', 'WA'), ('SA', 'SA'), ('TAS', 'TAS'), ('NT', 'NT'), ('ACT', 'ACT')], validators=[DataRequired()])
